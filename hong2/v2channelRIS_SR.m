...
% ============================================================
coef.lambda1 = (g1 .* hBI).' * phi;
coef.lambda2 = (g2 .* hBI).' * phi;
coef.lambdaE = (gE .* hBI).' * phi;

% ============================================================
% 2) Precompute path-loss scaling constants
% ============================================================
coef.K1  = P0 * (dIRS1)^(-m1) * (d0)^(-m1);
coef.K2  = P0 * (dIRS2)^(-m1) * (d0)^(-m1);
coef.KE  = P0 * (dRSE )^(-m1) * (d0)^(-m1);
%for C2 term in SIC denominator at U2

coef.KC2 = (dIRS2)^(-m1) * NBSa;
...
coef.B1 = P0 * (d1)^(-m2) * abs(hB1)^2;

% U2
coef.A2 = coef.K2 * abs(coef.lambda2)^2;
coef.B2 = P0 * (d2)^(-m2) * abs(hB2)^2;
